...
#define RT5680_DEBUG 1

//FIXME:by zx debug begin
static struct reg_sequence rt5680_init_list[] = {
	{ RT5680_MCLK_GATING_CTRL,	0x0001 },
	{ RT5680_AUTO_RC_CLK_CTRL1,	0x4000 }, //New Turn on GPIO debounce
	{ RT5680_PR_REG_VREF_CTRL1,	0x0853 },
	{ RT5680_LDO8_LDO9_PR_CTRL,	0x0F07 }, //Release LDO8 i_limit and adjust driving
	{ RT5680_DMIC_CTRL1,		0xE550 },
	{ RT5680_MF_PIN_CTRL1,		0xB419 },
	{ RT5680_MF_PIN_CTRL3,		0x5500 },
	{ RT5680_CLK_TREE_CTRL1,	0x0011 },
	{ RT5680_CLK_TREE_CTRL3,	0x0010 }, // New InBound3 SRC clock
	{ RT5680_I2S1_SDP,		0x8002 }, // PCM Mode A (LRCK One Plus at Master Mode)//FIXME:by zx
};
//FIXME:by zx debug end
...
static int rt5680_da_sto1_asrc_put(struct snd_kcontrol *kcontrol,
		struct snd_ctl_elem_value *ucontrol)
{
	...
	case 1 ... 7: /*enable*/
		if (snd_soc_read(codec, RT5680_PWR_DIG2) & RT5680_PWR_DAC_S1F)
			snd_soc_update_bits(codec, RT5680_ASRC2,
				RT5680_EN_STO_DAC_ASRC,	RT5680_EN_STO_DAC_ASRC);
		break;
	default: /*disable*/
		snd_soc_update_bits(codec, RT5680_ASRC2, RT5680_EN_STO_DAC_ASRC,
			0);
		break;
	}
...
static int rt5680_da_mono2l_asrc_put(struct snd_kcontrol *kcontrol,
		struct snd_ctl_elem_value *ucontrol)
{
	struct snd_soc_codec *codec = snd_kcontrol_chip(kcontrol);
	...
	switch (ucontrol->value.integer.value[0]) {
	case 1 ... 7: /*enable*/
		if (snd_soc_read(codec, RT5680_PWR_DIG2) & RT5680_PWR_DAC_M2F_L)
			snd_soc_update_bits(codec, RT5680_ASRC2,
				RT5680_EN_MONO_DAC_L_ASRC,
				RT5680_EN_MONO_DAC_L_ASRC);
		break;
	default: /*disable*/
		snd_soc_update_bits(codec, RT5680_ASRC2,
			RT5680_EN_MONO_DAC_L_ASRC, 0);
		break;
	}
...
static int rt5680_da_mono2r_asrc_put(struct snd_kcontrol *kcontrol,
		struct snd_ctl_elem_value *ucontrol)
{
	struct snd_soc_codec *codec = snd_kcontrol_chip(kcontrol);
	...
	switch (ucontrol->value.integer.value[0]) {
	case 1 ... 7: /*enable*/
		if (snd_soc_read(codec, RT5680_PWR_DIG2) & RT5680_PWR_DAC_M2F_R)
			snd_soc_update_bits(codec, RT5680_ASRC2,
				RT5680_EN_MONO_DAC_R_ASRC,
				RT5680_EN_MONO_DAC_R_ASRC);
		break;
	default: /*disable*/
		snd_soc_update_bits(codec, RT5680_ASRC2,
			RT5680_EN_MONO_DAC_R_ASRC, 0);
		break;
	}
...
static int rt5680_da_mono3l_asrc_put(struct snd_kcontrol *kcontrol,
		struct snd_ctl_elem_value *ucontrol)
{
	struct snd_soc_codec *codec = snd_kcontrol_chip(kcontrol);
	...
	switch (ucontrol->value.integer.value[0]) {
	case 1 ... 7: /*enable*/
		if (snd_soc_read(codec, RT5680_PWR_DIG2) & RT5680_PWR_DAC_M3F_L)
			snd_soc_update_bits(codec, RT5680_ASRC1,
				RT5680_EN_MONO_DA_3L_ASRC,
				RT5680_EN_MONO_DA_3L_ASRC);
		break;
	default: /*disable*/
		snd_soc_update_bits(codec, RT5680_ASRC1,
			RT5680_EN_MONO_DA_3L_ASRC, 0);
		break;
	}
...
static int rt5680_da_mono3r_asrc_put(struct snd_kcontrol *kcontrol,
		struct snd_ctl_elem_value *ucontrol)
{
	struct snd_soc_codec *codec = snd_kcontrol_chip(kcontrol);
	...
	switch (ucontrol->value.integer.value[0]) {
	case 1 ... 7: /*enable*/
		if (snd_soc_read(codec, RT5680_PWR_DIG2) & RT5680_PWR_DAC_M3F_R)
			snd_soc_update_bits(codec, RT5680_ASRC1,
				RT5680_EN_MONO_DA_3R_ASRC,
				RT5680_EN_MONO_DA_3R_ASRC);
		break;
	default: /*disable*/
		snd_soc_update_bits(codec, RT5680_ASRC1,
			RT5680_EN_MONO_DA_3R_ASRC, 0);
		break;
	}
...
static int rt5680_ad_sto1_asrc_put(struct snd_kcontrol *kcontrol,
		struct snd_ctl_elem_value *ucontrol)
{
	struct snd_soc_codec *codec = snd_kcontrol_chip(kcontrol);
	...
	switch (ucontrol->value.integer.value[0]) {
	case 1 ... 7: /*enable*/
		if (snd_soc_read(codec, RT5680_PWR_DIG2) & RT5680_PWR_ADC_S1F)
			snd_soc_update_bits(codec, RT5680_ASRC2,
				RT5680_EN_ADC_ASRC_STO1,
				RT5680_EN_ADC_ASRC_STO1);
		break;
	default: /*disable*/
		snd_soc_update_bits(codec, RT5680_ASRC2,
			RT5680_EN_ADC_ASRC_STO1, 0);
		break;
	}
...
static int rt5680_ad_sto2_asrc_put(struct snd_kcontrol *kcontrol,
		struct snd_ctl_elem_value *ucontrol)
{
	struct snd_soc_codec *codec = snd_kcontrol_chip(kcontrol);
	...
	switch (ucontrol->value.integer.value[0]) {
	case 1 ... 7: /*enable*/
		if (snd_soc_read(codec, RT5680_PWR_DIG2) & RT5680_PWR_ADC_S2F)
			snd_soc_update_bits(codec, RT5680_ASRC2,
				RT5680_EN_ADC_ASRC_STO2,
				RT5680_EN_ADC_ASRC_STO2);
		break;
	default: /*disable*/
		snd_soc_update_bits(codec, RT5680_ASRC2,
			RT5680_EN_ADC_ASRC_STO2, 0);
		break;
	}
...
static int rt5680_ad_sto3_asrc_put(struct snd_kcontrol *kcontrol,
		struct snd_ctl_elem_value *ucontrol)
{
	struct snd_soc_codec *codec = snd_kcontrol_chip(kcontrol);
	...
	switch (ucontrol->value.integer.value[0]) {
	case 1 ... 7: /*enable*/
		if (snd_soc_read(codec, RT5680_PWR_DIG2) & RT5680_PWR_ADC_S3F)
			snd_soc_update_bits(codec, RT5680_ASRC2,
				RT5680_EN_ADC_ASRC_STO3,
				RT5680_EN_ADC_ASRC_STO3);
		break;
	default: /*disable*/
		snd_soc_update_bits(codec, RT5680_ASRC2,
			RT5680_EN_ADC_ASRC_STO3, 0);
		break;
	}
...
static int rt5680_ad_monol_asrc_put(struct snd_kcontrol *kcontrol,
		struct snd_ctl_elem_value *ucontrol)
{
	struct snd_soc_codec *codec = snd_kcontrol_chip(kcontrol);
	...
	switch (ucontrol->value.integer.value[0]) {
	case 1 ... 7: /*enable*/
		if (snd_soc_read(codec, RT5680_PWR_DIG2) & RT5680_PWR_ADC_MF_L)
			snd_soc_update_bits(codec, RT5680_ASRC2,
				RT5680_EN_ADC_ASRC_MONOL,
				RT5680_EN_ADC_ASRC_MONOL);
		break;
	default: /*disable*/
		snd_soc_update_bits(codec, RT5680_ASRC2,
			RT5680_EN_ADC_ASRC_MONOL, 0);
		break;
	}
...
static int rt5680_ad_monor_asrc_put(struct snd_kcontrol *kcontrol,
		struct snd_ctl_elem_value *ucontrol)
{
	struct snd_soc_codec *codec = snd_kcontrol_chip(kcontrol);
	...
	switch (ucontrol->value.integer.value[0]) {
	case 1 ... 7: /*enable*/
		if (snd_soc_read(codec, RT5680_PWR_DIG2) & RT5680_PWR_ADC_MF_R)
			snd_soc_update_bits(codec, RT5680_ASRC2,
				RT5680_EN_ADC_ASRC_MONOR,
				RT5680_EN_ADC_ASRC_MONOR);
		break;
	default: /*disable*/
		snd_soc_update_bits(codec, RT5680_ASRC2,
			RT5680_EN_ADC_ASRC_MONOR, 0);
		break;
	}
...
static int rt5680_hp_vol_put(struct snd_kcontrol *kcontrol,
		struct snd_ctl_elem_value *ucontrol)
{
	struct snd_soc_codec *codec = snd_kcontrol_chip(kcontrol);
	int ret = snd_soc_put_volsw(kcontrol, ucontrol);

	if (snd_soc_read(codec, RT5680_STO_HP_NG2_CTRL1) & 0x8000) {
		snd_soc_update_bits(codec, RT5680_STO_HP_NG2_CTRL1, 0x8000,
			0x0000);
		snd_soc_update_bits(codec, RT5680_STO_HP_NG2_CTRL1, 0x8000,
			0x8000);
	}
...
static int rt5680_mono_vol_put(struct snd_kcontrol *kcontrol,
		struct snd_ctl_elem_value *ucontrol)
{
	struct snd_soc_codec *codec = snd_kcontrol_chip(kcontrol);
	int ret = snd_soc_put_volsw(kcontrol, ucontrol);

	if (snd_soc_read(codec, RT5680_MONO_AMP_NG2_CTRL1) & 0x8000) {
		snd_soc_update_bits(codec, RT5680_MONO_AMP_NG2_CTRL1, 0x8000,
			0x0000);
		snd_soc_update_bits(codec, RT5680_MONO_AMP_NG2_CTRL1, 0x8000,
			0x8000);
	}
...
static const struct snd_kcontrol_new rt5680_snd_controls[] = {
#ifdef RT5680_DEBUG //FIXME
	/*no use*/
#else
	/* Headphone Output Volume */
	SOC_DOUBLE_R_EXT_TLV("Headphone Playback Volume", RT5680_STO_HP_NG2_CTRL2,
		RT5680_STO_HP_NG2_CTRL3, RT5680_G_HP_SFT, 34, 1, snd_soc_get_volsw,
		rt5680_hp_vol_put, ng2_vol_tlv),

	/* Mono Output Volume */
	SOC_SINGLE_EXT_TLV("Mono Playback Volume", RT5680_MONO_AMP_NG2_CTRL2,
		RT5680_G_MONO_SFT, 34, 1, snd_soc_get_volsw, rt5680_mono_vol_put,
		ng2_vol_tlv),
#endif
	...
	/*no use*/
#else
	SOC_ENUM_EXT("DA STO1 ASRC Switch", rt5680_da_sto1_asrc_enum,
		snd_soc_get_enum_double, rt5680_da_sto1_asrc_put),
	SOC_ENUM_EXT("DA MONO2L ASRC Switch", rt5680_da_mono2l_asrc_enum,
		snd_soc_get_enum_double, rt5680_da_mono2l_asrc_put),
	SOC_ENUM_EXT("DA MONO2R ASRC Switch", rt5680_da_mono2r_asrc_enum,
		snd_soc_get_enum_double, rt5680_da_mono2r_asrc_put),
	SOC_ENUM_EXT("DA MONO3L ASRC Switch", rt5680_da_mono3l_asrc_enum,
		snd_soc_get_enum_double, rt5680_da_mono3l_asrc_put),
	SOC_ENUM_EXT("DA MONO3R ASRC Switch", rt5680_da_mono3r_asrc_enum,
		snd_soc_get_enum_double, rt5680_da_mono3r_asrc_put),
	SOC_ENUM_EXT("AD STO1 ASRC Switch", rt5680_ad_sto1_asrc_enum,
		snd_soc_get_enum_double, rt5680_ad_sto1_asrc_put),
	SOC_ENUM_EXT("AD STO2 ASRC Switch", rt5680_ad_sto2_asrc_enum,
		snd_soc_get_enum_double, rt5680_ad_sto2_asrc_put),
	SOC_ENUM_EXT("AD STO3 ASRC Switch", rt5680_ad_sto3_asrc_enum,
		snd_soc_get_enum_double, rt5680_ad_sto3_asrc_put),
	SOC_ENUM_EXT("AD MONOL ASRC Switch", rt5680_ad_monol_asrc_enum,
		snd_soc_get_enum_double, rt5680_ad_monol_asrc_put),
	SOC_ENUM_EXT("AD MONOR ASRC Switch", rt5680_ad_monor_asrc_enum,
		snd_soc_get_enum_double, rt5680_ad_monor_asrc_put),
	SOC_ENUM("OB 0-3 ASRC Switch", rt5680_ob_0_3_asrc_enum),
	SOC_ENUM("OB 4-7 ASRC Switch", rt5680_ob_4_7_asrc_enum),
#endif
...
static const struct snd_soc_dapm_route rt5680_dapm_routes[] = {
	...
	// {"AIF1TX", NULL, "IN"},
	#if 0//FIXME:by zx for debug
        { "OUT", NULL, "AIF1 Playback" },
	{ "AIF1 Capture", NULL, "IN" },
	#endif
...
#if 0 //ADD by XM for debug, by default ther is no rt5680_set_sysclk
static int rt5680_set_sysclk(struct snd_soc_dai *codec_dai,
				  int clk_id, unsigned int freq, int dir)
{
	struct snd_soc_codec *codec = codec_dai->codec;
	struct rt5680_priv *rt5680 = snd_soc_codec_get_drvdata(codec);
	rt5680->sysclk = freq;
	rt5680->sysclk_src = clk_id;

	printk("%s:%d: enter rt5680_set_sysclk()\n", __func__, __LINE__);
	return 0;
}
#endif
...
struct snd_soc_dai_ops rt5680_aif_dai_ops = {
	...
	//.set_sysclk = rt5680_set_sysclk,//by XM
	.set_tdm_slot = rt5680_set_tdm_slot,//FIXME
};
...
{
	int codec_power_enable_gpio = -1;//for CODEC_AVDD_1V8_EN
		    ...
		    const struct i2c_device_id *id)
{
	...
	/* audio add: rt5680 1.8v enable pin */
	if (np) {
...
#if 1 //by XM for gpio init late
module_i2c_driver(rt5680_i2c_driver);
#else
static int __init rt5680_i2c_driver_init(void)
{
    return i2c_add_driver(&rt5680_i2c_driver);
}
late_initcall(rt5680_i2c_driver_init);

static void __exit rt5680_i2c_driver_exit(void)
{
    return i2c_del_driver(&rt5680_i2c_driver);
}
module_exit(rt5680_i2c_driver_exit);
#endif
