...
static int fwu_get_image_firmware_id(unsigned int *fw_id)
{
	...
	if (fwu->img.contains_firmware_id) {
		*fw_id = fwu->img.firmware_id;
		...
		max_index = (unsigned long)fwu->image_name;
		max_index += (MAX_IMAGE_NAME_LEN - 1);
		max_index -= (unsigned long)strptr;
		...
		max_index = min(max_index,
				(unsigned long)(MAX_FIRMWARE_ID_LEN - 2));
