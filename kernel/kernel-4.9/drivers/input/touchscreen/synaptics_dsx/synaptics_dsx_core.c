		...
		struct synaptics_rmi4_fn *fhandler)
{
	...
	unsigned char swipe_info = 0;
	unsigned char swipe_finger_num = 0;
#endif
	...
	if (gesture_type) {//debug
		dev_err(rmi4_data->pdev->dev.parent,
					"%s: gesture_type[0x%02x]\n",
					__func__, gesture_type);
	}

	if (gesture_type == LPWG_SINGLETAP_DETECTED) {
		//debug
		dev_err(rmi4_data->pdev->dev.parent,
				"%s: single tap\n",
				__func__);
		//TODO: add xiaomi logic
		input_report_key(rmi4_data->input_dev,
					GESTURE_XM_ADDR+LPWG_SINGLETAP_DETECTED, 1);
		input_sync(rmi4_data->input_dev);
		input_report_key(rmi4_data->input_dev,
					GESTURE_XM_ADDR+LPWG_SINGLETAP_DETECTED, 0);
		input_sync(rmi4_data->input_dev);
	} else if (gesture_type == LPWG_DOUBLETAP_DETECTED) {
		//debug
		dev_err(rmi4_data->pdev->dev.parent,
				"%s: double tap\n",
				__func__);
		//TODO: add xiaomi logic
		input_report_key(rmi4_data->input_dev,
					GESTURE_XM_ADDR+LPWG_DOUBLETAP_DETECTED, 1);
		input_sync(rmi4_data->input_dev);
		input_report_key(rmi4_data->input_dev,
					GESTURE_XM_ADDR+LPWG_DOUBLETAP_DETECTED, 0);
		input_sync(rmi4_data->input_dev);
	} else if (gesture_type == LPWG_TOUCHANDHOLD_DETECTED) {
		//debug
		dev_err(rmi4_data->pdev->dev.parent,
				"%s: touch and hold\n",
				__func__);
		//TODO: add xiaomi logic
		input_report_key(rmi4_data->input_dev,
					GESTURE_XM_ADDR+LPWG_TOUCHANDHOLD_DETECTED, 1);
		input_sync(rmi4_data->input_dev);
		input_report_key(rmi4_data->input_dev,
					GESTURE_XM_ADDR+LPWG_TOUCHANDHOLD_DETECTED, 0);
		input_sync(rmi4_data->input_dev);
	} else if ((gesture_type >= LPWG_SWIPE_DETECTED) && (gesture_type <= LPWG_SWIPE_DETECTED_RIGHT_CONTINU)){
		retval = synaptics_rmi4_reg_read(rmi4_data,
				GESTURE_XM_ADDR0,
				&swipe_info,
				sizeof(swipe_info));
		if (retval < 0) {
			dev_err(rmi4_data->pdev->dev.parent,
					"%s: Failed to read swipe info data registers\n",
					__func__);
		}

		/* single finger or double fingers */
		swipe_finger_num = 0;
		if ((swipe_info & LPWG_SWIPE_FINGER_NUM_MASK) == LPWG_SWIPE_ID_SINGLE) {
			swipe_finger_num = 1;
			//TODO: add xiaomi logic
		} else if ((swipe_info & LPWG_SWIPE_FINGER_NUM_MASK) == LPWG_SWIPE_ID_DOUBLE) {
			swipe_finger_num = 2;
			//TODO: add xiaomi logic
		} else {
			dev_err(rmi4_data->pdev->dev.parent,
					"%s: incorrect finger number for swipe\n",
					__func__);
		}

		//debug
		dev_err(rmi4_data->pdev->dev.parent,
				"%s: swipe_info:0x%02x; finger num: 0x%02x; swipe dir: %s %s %s %s \n",
				__func__,
				swipe_info,
				swipe_finger_num,
				((swipe_info & LPWG_SWIPE_FINGER_UP_DOWN_DIR_MASK) == LPWG_SWIPE_UP) ? ("right") : ("  "),
				((swipe_info & LPWG_SWIPE_FINGER_UP_DOWN_DIR_MASK) == LPWG_SWIPE_DOWN) ? ("left") : ("    "),
				((swipe_info & LPWG_SWIPE_FINGER_LEFT_RIGHT_DIR_MASK) == LPWG_SWIPE_LEFT) ? ("down") : ("    "),
				((swipe_info & LPWG_SWIPE_FINGER_LEFT_RIGHT_DIR_MASK) == LPWG_SWIPE_RIGHT) ? ("up") : ("     ")
				);

		/* up or down */
		if ((swipe_info & LPWG_SWIPE_FINGER_UP_DOWN_DIR_MASK) == LPWG_SWIPE_UP) {
			//TODO: add xiaomi logic
			input_report_key(rmi4_data->input_dev,
						GESTURE_XM_ADDR+LPWG_SWIPE_RIGHT, 1);
			input_sync(rmi4_data->input_dev);
			input_report_key(rmi4_data->input_dev,
						GESTURE_XM_ADDR+LPWG_SWIPE_RIGHT, 0);
			input_sync(rmi4_data->input_dev);
		} else if ((swipe_info & LPWG_SWIPE_FINGER_UP_DOWN_DIR_MASK) == LPWG_SWIPE_DOWN) {
			//TODO: add xiaomi logic
			input_report_key(rmi4_data->input_dev,
						GESTURE_XM_ADDR+LPWG_SWIPE_LEFT, 1);
			input_sync(rmi4_data->input_dev);
			input_report_key(rmi4_data->input_dev,
						GESTURE_XM_ADDR+LPWG_SWIPE_LEFT, 0);
			input_sync(rmi4_data->input_dev);
		}

		/* left or right */
		if ((swipe_info & LPWG_SWIPE_FINGER_LEFT_RIGHT_DIR_MASK) == LPWG_SWIPE_LEFT) {
			//TODO: add xiaomi logic
			input_report_key(rmi4_data->input_dev,
						GESTURE_XM_ADDR+LPWG_SWIPE_DOWN, 1);
			input_sync(rmi4_data->input_dev);
			input_report_key(rmi4_data->input_dev,
						GESTURE_XM_ADDR+LPWG_SWIPE_DOWN, 0);
			input_sync(rmi4_data->input_dev);
		} else if ((swipe_info & LPWG_SWIPE_FINGER_LEFT_RIGHT_DIR_MASK) == LPWG_SWIPE_RIGHT) {
			//TODO: add xiaomi logic
			input_report_key(rmi4_data->input_dev,
						GESTURE_XM_ADDR+LPWG_SWIPE_UP, 1);
			input_sync(rmi4_data->input_dev);
			input_report_key(rmi4_data->input_dev,
						GESTURE_XM_ADDR+LPWG_SWIPE_UP, 0);
			input_sync(rmi4_data->input_dev);
		}
...
static irqreturn_t synaptics_rmi4_irq(int irq, void *data)
{
	...
	if (gpio_get_value_cansleep(bdata->irq_gpio) != bdata->irq_on_state)
		goto exit;
...
#ifdef F51_DISCRETE_GESTURE_XM
	set_bit(GESTURE_XM_ADDR+LPWG_SINGLETAP_DETECTED, rmi4_data->input_dev->keybit);
	set_bit(GESTURE_XM_ADDR+LPWG_DOUBLETAP_DETECTED, rmi4_data->input_dev->keybit);
	set_bit(GESTURE_XM_ADDR+LPWG_TOUCHANDHOLD_DETECTED, rmi4_data->input_dev->keybit);
	set_bit(GESTURE_XM_ADDR+LPWG_CIRCLE_DETECTED, rmi4_data->input_dev->keybit);
	set_bit(GESTURE_XM_ADDR+LPWG_VEE_DETECTED, rmi4_data->input_dev->keybit);
	set_bit(GESTURE_XM_ADDR+LPWG_UNICODE_DETECTED, rmi4_data->input_dev->keybit);
	set_bit(GESTURE_XM_ADDR+LPWG_SWIPE_DETECTED, rmi4_data->input_dev->keybit);
	set_bit(GESTURE_XM_ADDR+LPWG_SWIPE_DETECTED_UP_CONTINU, rmi4_data->input_dev->keybit);
	set_bit(GESTURE_XM_ADDR+LPWG_SWIPE_DETECTED_DOWN_CONTINU, rmi4_data->input_dev->keybit);
	set_bit(GESTURE_XM_ADDR+LPWG_SWIPE_DETECTED_LEFT_CONTINU, rmi4_data->input_dev->keybit);
	set_bit(GESTURE_XM_ADDR+LPWG_SWIPE_DETECTED_RIGHT_CONTINU, rmi4_data->input_dev->keybit);
	set_bit(GESTURE_XM_ADDR+LPWG_SWIPE_UP, rmi4_data->input_dev->keybit);
	set_bit(GESTURE_XM_ADDR+LPWG_SWIPE_DOWN, rmi4_data->input_dev->keybit);
	set_bit(GESTURE_XM_ADDR+LPWG_SWIPE_LEFT, rmi4_data->input_dev->keybit);
	set_bit(GESTURE_XM_ADDR+LPWG_SWIPE_RIGHT, rmi4_data->input_dev->keybit);
#endif
...
static int synaptics_rmi4_set_gpio(struct synaptics_rmi4_data *rmi4_data)
{
	...
	if (bdata->power_gpio >= 0) {
		gpio_set_value_cansleep(bdata->power_gpio, bdata->power_on_state);
		msleep(bdata->power_delay_ms);
	}

	if (bdata->reset_gpio >= 0) {
		gpio_set_value_cansleep(bdata->reset_gpio, bdata->reset_on_state);
