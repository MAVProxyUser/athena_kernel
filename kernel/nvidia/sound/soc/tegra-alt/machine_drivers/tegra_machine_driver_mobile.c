...
static const struct snd_soc_dapm_widget tegra_machine_dapm_widgets[] = {
	...
	/* 5680 dmic */
	SND_SOC_DAPM_SPK("h1 DMIC", NULL),//FIXME:by zx for debug
				  ...
				  u64 formats)
{
	...
	//by XM
	dev_info(card->dev, "enter tegra_machine_dai_init()\n");
...
//Audio Add Start
	rtd = snd_soc_get_pcm_runtime(card, "tas5805-codec");
	if (rtd) {
	...
	rtd = snd_soc_get_pcm_runtime(card, "rt5680-codec");
	if (rtd) {
		dev_info(card->dev, "rt5680 pcm runtime\n");
		dai_params =
		(struct snd_soc_pcm_stream *)rtd->dai_link->params;
	...
	err = snd_soc_dai_set_tdm_slot(rtd->codec_dai, 0x3f, 0x0, 6, 16);//FIXME: by zx
	if (err < 0) {
		dev_err(card->dev, "Can't configure rt5680 TDM: %d\n", err);
		return err;
	}
//Audio Add End
...
static struct snd_soc_card snd_soc_tegra_card = {
	...
	//.fully_routed = true, //FIXME:by zx for debug
};
